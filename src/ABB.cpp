...
bool ABB::insercao(const tipo valor) {
    if (raiz == nullptr) {
        ...
        return insercao(raiz, valor);
    }
}

// Inserção em árvore não vazia
bool ABB::insercao(Node* node, const tipo valor) {
	...
	if (dado != valor) {
        if (dado < valor) { // Inserir do lado direito
            if (node->dir == nullptr) {
                ...
                ++node->dir_cnt;
                return true;
            } else if (insercao(node->dir, valor)) {
                ++node->dir_cnt;
                return true;
            }
            ...
            if (node->esq == nullptr) {
                ...
                ++node->esq_cnt;
                return true;
            } else if (insercao(node->esq, valor)) {
                ++node->esq_cnt;
                return true;
            }
        }
